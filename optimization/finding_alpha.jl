...
for j in 1:length(radiusRange)
    ...
    for j in 1:length(thresholds)
        index=argmin(alld[j,v,:])
        index_eu=argmin(alld_euclidean[:,j])
        @load "/user/aurossi/home/mri_networks/hyperbolicbrains/optimization/hyperbolic_300_alpha_$(round(Int,αrange[index]*100))_R_05_05_14_v_01_01_09_correct.jld2" avHY clHY plHY tccHY 
        @load "/user/aurossi/home/mri_networks/hyperbolicbrains/optimization/euclidean_300_thre_02_005_098.jld2" avEU clEU plEU tccEU 
        varvect=clHY[v,:]./plHY[v,:]
        spline=Spline1D(sort(reverse(avHY[v,:])),reverse(varvect))
        spline_eu=Spline1D(sort(reverse(avEU[index_eu,:])),reverse(clEU[index_eu,:]./plEU[index_eu,:]))
        if avquatile[j]<+165
        p=scatter!([avquatile[j]], [evaluate(spline,avquatile[j])],color=:red,alpha=.5,markershape=:star5,label="Hyperbolic alpha=$(αrange[index]),speed=$(velocities[v])")
        p=scatter!([avquatile[j]], [evaluate(spline_eu,avquatile[j])],color=:green,alpha=.5,markershape=:rect,
        label="Euclidean speed=$(velocities[index_eu])", 
        xlims=(0,165))
        end
    ...
    display(p)
    #plot!(size=(1300,1000))
    Plots.html(p, "/user/aurossi/home/mri_networks/hyperbolicbrains/optimization/images/settingvelocity_$(velocities[v])")
...
for j in 1:length(thresholds)
    index=argmin(alld[j,v,:])
     pa=Plots.scatter!([avquatile[j]],[αrange[index]],
    ylims=(0,1.4),
    xlims=(0,165),
    xlabel="Average degree",
    ylabel="Alpha",
    title="Alphas setting velocity=$(velocities[v])",
    legend=false,
    dpi=900)
end

Plots.png(pa, "/user/aurossi/home/mri_networks/hyperbolicbrains/optimization/bestalpha_velocity_$(velocities[v])")
...
Plots.png(pv, "/user/aurossi/home/mri_networks/hyperbolicbrains/optimization/bestvelocities_allalpha")
end
