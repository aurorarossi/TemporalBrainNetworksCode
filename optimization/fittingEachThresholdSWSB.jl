...
function plot_hyperbolic(thresholds,velocities,αrange, average_deg_quantile, small_world_SB_quantile, p)
    ...
    for a in 1:length(αrange)
        ...
        for i in 1:length(velocities)
            ...
            splineorg=Spline1D(sort(reverse(average_deg_quantile[:])),reverse(small_world_SB_quantile[2,:]))
    ...
    for j in 1:length(thresholds)-1
        ...
        varvect=tccHY[index[1],:]./plHY[index[1],:]
    ...
    varvect=tccHY[index[1],:]./plHY[index[1],:]
    spline=Spline1D(sort(reverse(avHY[index[1],:])),reverse(varvect))
    scatter!(p,[average_deg_quantile[end]], [evaluate(spline,average_deg_quantile[end])],color=1,alpha=0.75,markersize=4,markerstrokewidth=0.6,markershape=:diamond, label=L"\mathrm{Hyperbolic \,\, }")

end

function plot_euclidean(thresholds,velocities, average_deg_quantile, small_world_SB_quantile,p)
    ...
    for i in 1:length(velocities)
            spline=Spline1D(sort(reverse(avEU[i,:])),reverse(tccEU[i,:]./plEU[i,:]))
            splineorg=Spline1D(sort(reverse(average_deg_quantile[:])),reverse(small_world_SB_quantile[2,:]))
            for j in 1:length(thresholds)
    ...
    for j in 1:length(thresholds)-1
        ...
        @load "/user/aurossi/home/mri_networks/hyperbolicbrains/optimization/euclidean_300_thre_02_005_098.jld2" avEU clEU plEU tccEU 
        spline_eu=Spline1D(sort(reverse(avEU[index_eu,:])),reverse(tccEU[index_eu,:]./plEU[index_eu,:]))
    ...
    @load "/user/aurossi/home/mri_networks/hyperbolicbrains/optimization/euclidean_300_thre_02_005_098.jld2" avEU clEU plEU tccEU 
    spline_eu=Spline1D(sort(reverse(avEU[index_eu,:])),reverse(tccEU[index_eu,:]./plEU[index_eu,:]))
    scatter!(p,[average_deg_quantile[end]], [evaluate(spline_eu,average_deg_quantile[end])],color=5,alpha=0.75,markersize=4,markerstrokewidth=0.6,markershape=:rect, label= L"\mathrm{Random \,\, torus \,\,}")
end

function plot_euclideanBorder(thresholds,velocities,radiusRange, average_deg_quantile, small_world_SB_quantile,p)
    ...
    for i in 1:length(velocities)
            spline=Spline1D(sort(avEU[i,:]),tccEU[i,:]./plEU[i,:])
            splineorg=Spline1D(sort(reverse(average_deg_quantile[:])),reverse(small_world_SB_quantile[2,:]))
    ...
    for j in 1:length(radiusRange)-1
        ...
        @load "/user/aurossi/home/mri_networks/hyperbolicbrains/optimization/euclideanBorder_300_r_0_005_08.jld2" avEU clEU plEU tccEU 
        spline_euB=Spline1D(sort(avEU[index_euB,:]),tccEU[index_euB,:]./plEU[index_euB,:])
    ...
    @load "/user/aurossi/home/mri_networks/hyperbolicbrains/optimization/euclideanBorder_300_r_0_005_08.jld2" avEU clEU plEU tccEU 
    spline_euB=Spline1D(sort(avEU[index_euB,:]),tccEU[index_euB,:]./plEU[index_euB,:])
    scatter!(p,[average_deg_quantile[end]], [evaluate(spline_euB,average_deg_quantile[end])],color=3,alpha=0.75,markersize=4,markerstrokewidth=0.6,markershape=:hexagon,label=L"\mathrm{Random \,\, square \,\,}")
end
...
function main()
    ...
    p = Plots.scatter(average_deg_quantile,small_world_SB_quantile[2,:],c=2, palette=:Set1_9, legendfontsize=12, xlims=(0, 170), grid=false,xticks = ([0, 50, 100, 150, 200], [L"0", L"50", L"100", L"150", L"200"]), yticks = ([0, 0.25, 0.5,0.75,1], [L"0", L"0.25", L"0.5", L"0.75",L"1"]),markersize=5.3,markerstrokewidth=0.6,label=L"\mathrm{Real\,\, data\,\,  Schaefer}", dpi=1200, legend=false)
    ylabel!(L"\mathrm{Temporal\,\, small\,\, worldness \,\,} S_{SB}")
    xlabel!(L"\mathrm{Average\,\,  degree}")
    plot_euclideanBorder(thresholds,velocities,radiusRange, average_deg_quantile, small_world_SB_quantile,p)
    plot_hyperbolic(thresholds,velocities,αrange, average_deg_quantile, small_world_SB_quantile, p)
    plot_euclidean(thresholds,velocities, average_deg_quantile, small_world_SB_quantile,p)
    display(p)
    savefig(p, "/user/aurossi/home/mri_networks/TemporalBrainNetworksCode/images/swSBEachThreshold.png")
end
